...
<!--        <p style="color: #FFFFFF;">{{formDetails}}</p>-->
        <div class="center">
          <p class="text-1"> <span>Unlimited</span> </p>
        </div>

        <!--          <p class="text-2">(Driving License or Government ID card)</p>-->

        <div style="display: flex;">
          <img width="40" height="40" src="https://img.icons8.com/office/40/money--v1.png" alt="money--v1"/>
          <p class="text-3">You are Eligible for unlimited day trades as your current net account value is over $25,000.</p>
...
<!--          <button class="btn-2"  @click="close">Place trade</button>-->
          <button class="btn" @click="placeTrade" >Proceed to place trade</button>
        </div>
...
export default {
  ...
  computed:{
    ...
    formDetails() {
      return StoreUtils.rootGetters(StoreUtils.getters.auth.getIsModalOpened2)
    },
  },
  methods: {
    close(){
    ...
    async placeTrade() {
      ...
      await StoreUtils.dispatch(StoreUtils.actions.trade.tradeCreate, {
        ...
        tradeTime: this.formDetails.tradeTime,
        symbolTraded: this.formDetails.symbolTraded,
        amountTrade: this.formDetails.amountTrade,
        expectedPayout: 0,
        leverage: this.formDetails.leverage,
        endPrice: this.formDetails.endPrice,
        marketType: this.formDetails.marketType,
        endTime: this.formDetails.endTime,
        tradeStatus: "pending",
        tradeReference: this.formDetails.tradeReference,
        tradeType: this.formDetails.tradeType
      });
